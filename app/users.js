  ...
  async getTopCustomers(event) {
    try {
      ...
      const customersWithSpend = await customers.map(async (customer) => {
        ...
        ;[customer.dataValues.spend, customer.dataValues.loadsPerMonth] =
          await this.helpers.getCustomerSpendAndLoadCount(customer, status)

        return customer
      })

      const customersResolved = await Promise.all(customersWithSpend)
      const topCustomers = [...customersResolved].sort((a, b) => {
