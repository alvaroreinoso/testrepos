...
module.exports.createStripeSubscription = async (event, context) => {
  ...
  try {
    ...
    let updateCustomerDefaultPaymentMethod = await stripe.customers.update(reqBody.customerId, {
      invoice_settings: {
...
module.exports.updateSubscription = async (event, context) => {
  ...
  try {
    ...
    const subscription = await stripe.subscriptions.update(brokerage.stripeSubscriptionId, request)
