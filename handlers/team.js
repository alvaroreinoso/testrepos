...
module.exports.getTeammatesForTeam = async (event, context) => {
  ...
  try {
    ...
    const teammatesWithSpend = await Promise.all(
      await teammates.map(async (teammate) => {
        ...
        const spendPerMonth = await lanes.reduce((a, b) => ({
    ...
    const sortedTeammates = await teammatesWithSpend.sort((a, b) => {
      return b.dataValues.spendPerMonth - a.dataValues.spendPerMonth
    })
...
module.exports.getLanesForTeam = async (event, context) => {
  ...
  try {
    ...
    if (lanes.length == 0) {
      ...
      const loadsPerMonth = await lanesResolved.reduce((a, b) => ({
        currentVolume: a.currentVolume + b.currentVolume,
      }))
...
module.exports.editTeam = async (event, context) => {
  ...
  try {
    ...
    ;(team.name = request.name), (team.icon = request.icon)
