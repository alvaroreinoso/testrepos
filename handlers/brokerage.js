...
module.exports.getBrokerage = async (event, context) => {
  ...
  const topTeams = teams.sort((a, b) => {
    return b.dataValues.revenue - a.dataValues.revenue
  })
  const topTeammates = teammatesWithCount.sort((a, b) => {
    return b.dataValues.revenue - a.dataValues.revenue
  })
  ...
  try {
    ...
    for (const location of locations) {
      const locationLanes = await Lane.findAll({
        where: {
          [Op.or]: [{ originLocationId: location.id }, { destinationLocationId: location.id }],
        },
