    ...
    switch (path) {
      case '/api/search': {
        const searchResults = await client.search({
          index: ['customer', 'user', 'lane', 'customer_location'],
          body: {
            query: {
              bool: {
                must: [
                ...
                filter: [{ term: { brokerageId: user.brokerageId } }],
              },
            },
          },
        })
      ...
      case '/api/search/contacts': {
        ...
        const contactsAvailable = await filterSearchResultsForItem(
          contactsFromSearch,
          contactsInItem
        )
