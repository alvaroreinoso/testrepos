...
describe('Test Get Lanes by current user lambda', () => {

    test('Get lanes with no auth', async () => {
        ...
        const response = await laneHandler.getLanesByCurrentUser(request)

        expect(response.statusCode).toStrictEqual(401)
        expect(response.body).toBeUndefined()
    })

    test('Get lanes for user with no lanes', async () => {
    ...
    test('Get lanes for user with lanes', async () => {
